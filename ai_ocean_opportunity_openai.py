...
def _raise_for_response_issues(response: Any) -> None:
    ...
    if status == "incomplete":
        ...
        raise OpenAIResponseError(f"OpenAI response was incomplete: {reason or 'unknown'}")
...
def _parse_json_output(response: Any) -> dict[str, Any]:
    ...
    if not isinstance(parsed, dict):
        raise OpenAIResponseError("OpenAI JSON output must be an object at the top level.")
